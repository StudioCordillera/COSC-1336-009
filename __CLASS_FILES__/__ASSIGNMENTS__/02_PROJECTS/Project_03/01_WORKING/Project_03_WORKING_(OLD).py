...
# 2: "AllyBaba Bank charges a flat fee of $10 per month for maintaining a checking account." | 4: Add month handling list/func
...
def calculateFees(checks):
    ...
    # 2 | Add 10$ flat fee per month < TODO
    ...
    if (checks < 20):
        ...
        # 3 | Need to format to 2 decimal places when displaying summary < TODO
...
def shenanigans():
    ...
    while (True):
        ...
        if (month.upper() in monthList):
            indexMonth = monthList.index(month)+1
            month = month.upper()
            break

        else:
            print('Months in \'JAN\' format please...')
    ...
    return month, checks, indexMonth
...
def monthHandler(month):
    # 4: Add month handling list/func < TODO
...
def main(): 
    ...
    month, checks, indexMonth = shenanigans()
